...
<section class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="p-4 border-b border-slate-100 flex items-center justify-between">
        <form method="GET" action="{{ route('kondisi.index') }}" class="relative w-72">
            <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400">
                <i class="w-4 h-4" data-lucide="search"></i>
            </span>
            <input name="search" value="{{ request('search') }}" class="block w-full pl-10 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Cari kondisi barang..." type="text"/>
        </form>
        </div>
                ...
                <tr class="{{ $loop->even ? 'bg-slate-50/30' : '' }} table-row-hover transition-colors">
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex justify-center space-x-3">
                            ...
                            <form action="{{ route('kondisi.destroy', $kondisi->id) }}" method="POST" data-confirm="Yakin ingin menghapus kondisi ini?" data-ajax-delete="true" class="inline">
                                @csrf
                                @method('DELETE')
