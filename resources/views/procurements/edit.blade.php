...
@section('title', 'Edit Pengajuan ' . $procurement->code)
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
        <div class="flex items-center gap-2">
            <a href="{{ route('procurements.show', $procurement->id) }}" class="p-1.5 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors">
                <span class="material-symbols-outlined text-[20px]">arrow_back</span>
            </a>
            <h2 class="text-2xl font-bold text-gray-800">Edit Pengajuan {{ $procurement->code }}</h2>
        </div>
        <p class="text-sm text-gray-500 mt-1 ml-9">Perbarui rincian usulan pengadaan alat</p>
    </div>
</div>

@if($procurement->status === 'rejected' && $procurement->rejection_reason)
<div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-2xl flex items-start gap-3 text-red-800 text-sm">
    <span class="material-symbols-outlined text-red-600 text-[24px] flex-shrink-0">info</span>
    <div>
        <h4 class="font-bold text-red-900">Catatan Penolakan Sebelumnya:</h4>
        <p class="mt-0.5 text-xs text-red-700 leading-relaxed">{{ $procurement->rejection_reason }}</p>
        <p class="mt-1 text-[11px] text-red-600 italic">Silakan perbaiki data di bawah ini lalu klik "Ajukan Kembali".</p>
    </div>
</div>
@endif
...
<form action="{{ route('procurements.update', $procurement->id) }}" method="POST" enctype="multipart/form-data" id="procurementForm" class="space-y-6">
    @csrf
    @method('PUT')
    <input type="hidden" name="action_type" id="actionTypeInput" value="draft">
    ...
    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" value="{{ old('title', $procurement->title) }}" required
            ...
            <div>
                ...
                @if(Auth::user()->isJurusan())
                    <input type="hidden" name="jurusan_id" value="{{ $procurement->jurusan_id }}">
                    <div class="px-3.5 py-2.5 text-sm bg-gray-100 border border-gray-200 rounded-xl text-gray-700 font-medium">
                        {{ $procurement->jurusan->nama_jurusan ?? 'Pusat NOC' }}
                    </div>
                @else
            ...
            <div>
                ...
                <select name="priority" required class="w-full px-3.5 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3F51B5]/30 focus:border-[#3F51B5]">
                    <option value="sedang" {{ old('priority', $procurement->priority) === 'sedang' ? 'selected' : '' }}>🔹 Sedang</option>
                    <option value="tinggi" {{ old('priority', $procurement->priority) === 'tinggi' ? 'selected' : '' }}>⚡ Tinggi</option>
                    <option value="mendesak" {{ old('priority', $procurement->priority) === 'mendesak' ? 'selected' : '' }}>🚨 Mendesak</option>
                    <option value="rendah" {{ old('priority', $procurement->priority) === 'rendah' ? 'selected' : '' }}>⚪ Rendah</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="target_date" value="{{ old('target_date', $procurement->target_date ? $procurement->target_date->format('Y-m-d') : '') }}"
                       class="w-full px-3.5 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3F51B5]/30 focus:border-[#3F51B5]">
            </div>
            ...
            <div>
                ...
                <div class="mt-1 flex items-center gap-1.5 text-xs text-blue-600">
                    <span class="material-symbols-outlined text-[15px]">attach_file</span>
                    <a href="{{ asset('storage/' . $procurement->attachment) }}" target="_blank" class="hover:underline">Lihat lampiran saat ini</a>
                </div>
                @endif
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="justification" rows="3"
                          class="w-full px-3.5 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3F51B5]/30 focus:border-[#3F51B5]">{{ old('justification', $procurement->justification) }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-3 border-b border-gray-100 gap-2">
            ...
            <button type="button" onclick="addNewItemRow()" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-[#3F51B5] hover:bg-blue-100 text-xs font-bold rounded-xl transition-colors">
        ...
        <div class="mt-6 p-4 bg-gray-50 rounded-xl border border-gray-200/80 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <h5 id="totalItemsCountText" class="text-sm font-bold text-gray-800">0 Barang</h5>
                </div>
            </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-2">
        <a href="{{ route('procurements.show', $procurement->id) }}" class="w-full sm:w-auto px-5 py-2.5 text-xs font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors text-center">
            Batal
        </a>
...
<script>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-center pt-2 border-t border-gray-50">
                    ...
                    <div class="md:col-span-4">
                        <input type="url" name="items[${index}][reference_url]" value="${data ? data.reference_url || '' : ''}"
                               placeholder="Link Referensi / Toko Online" 
                               class="w-full px-3 py-1.5 text-xs bg-gray-50/70 border border-gray-200 rounded-lg text-gray-600 focus:outline-none">
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @else
            const existingItems = @json($procurement->items);
            if (existingItems && existingItems.length > 0) {
                existingItems.forEach(item => addNewItemRow(item));
            } else {
                addNewItemRow();
            }
        @endif
